...
namespace unlockfps_nc.Service
{
    public class ProcessService
    {
        ...
        private async Task Worker ( )
        {
            ...
            var unlockerPath = Application.StartupPath;
            ...
            /*
            * Changes here are intended to provide hardcoded and foolproof injection of both 3dmigoto and reshade. Inject function now can be overloaded to take a single module instead of a full list which I initialy thought fixed an inherent bug
            */

           
            List<string> parsedDlls = new List<String> ( );
            var debugOverride = true;
            //Use provided 3dmigoto library 
            var migotoLib = Path.Combine ( unlockerPath, "3DM/d3d11.dll" );
       

            //Treat the end user like a baby and deny injection of conflicting modules
            
            foreach ( string dllPath in _config.DllList ) {
               if ( !dllPath.Contains ( "Shade64.dll" ) && !dllPath.Contains ( "d3d11.dll" ) ) 
                    {
                    parsedDlls.Add ( dllPath );
                }
            }
    

             if ( !ProcessUtils.InjectDlls ( pi.hProcess, migotoLib ) )
            {
                MessageBox.Show (
                            $@"3DMigoto Dll Injection failed ({Marshal.GetLastWin32Error ( )}){Environment.NewLine} {Marshal.GetLastPInvokeErrorMessage ( )}",
                            @"Error", MessageBoxButtons.OK, MessageBoxIcon.Error );
            }


            /*
          //inject anything else. Not sure what you'd even need to inject and if this will still work with the changed addresses but I'll leave it. Special K can be injected if you inject specialk64.dll here and then initate the global service from the front end. I don't think its very useful and it does crash the game often but the option is there 

          if ( parsedDlls.Count > 0 ) {
              if ( !ProcessUtils.InjectDlls ( pi.hProcess, parsedDlls ) )
              {
                  MessageBox.Show (
          ...
          */
            ...
            var ReshadePath = Path.Combine ( unlockerPath, "ReShade64.dll" );
            var GShadePath = Path.Combine ( unlockerPath, "GShade64.dll" );

            var shadePath = _config.UseGShadeFallback ? GShadePath : ReshadePath;

            if ( !ProcessUtils.InjectDlls ( pi.hProcess, shadePath ))
            {
                MessageBox.Show (
                            $@"ReShade Injection failed ({Marshal.GetLastWin32Error ( )}){Environment.NewLine} {Marshal.GetLastPInvokeErrorMessage ( )}",
                            @"Error", MessageBoxButtons.OK, MessageBoxIcon.Error );
            }
